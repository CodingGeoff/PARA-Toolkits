...
# --- Settings Dialog Window (Contains a key fix) ---
class SettingsDialog(QDialog):
    ...
    def browse_directory(self):
        ...
        if directory:
            # --- FIX ---
            # Removed the .replace("\\", "/") to store the native path
...
class ParaFileManager(QMainWindow):
    ...
    def reload_configuration(self):
        ...
        try:
            with open(resource_path("config.json"), "r") as f:
                ...
                # Normalize path right after loading
                if path: new_base_dir = os.path.normpath(path)
                if not new_base_dir or not os.path.isdir(os.path.dirname(new_base_dir)): raise ValueError(f"Path '{path}' is invalid or parent doesn't exist.")
    ...
    def show_context_menu(self, pos):
        ...
        menu.addSeparator(); rename_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_FileLinkIcon), "Rename..."); delete_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon), "Delete...")
    ...
    def open_selected_file(self, item=None):
        ...
        if not item: return
        path = os.path.normpath(item.data(Qt.ItemDataRole.UserRole)) # Normalize path
    ...
    def rename_file(self):
        list_widget, item = self.get_current_list_and_item()
        if not item: return
        old_path = os.path.normpath(item.data(Qt.ItemDataRole.UserRole)) # Normalize path
    ...
    def delete_file(self):
        ...
        if not os.path.exists(path):
            self.log(f"ERROR: File '{filename}' no longer exists."); self.refresh_file_list(list_widget.category_name); return
        reply = QMessageBox.warning(self, "Confirm Action", f"Are you sure you want to move this file to the Recycle Bin?\n\n'{filename}'", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
                send2trash.send2trash(path)
                self.log(f"Moved '{filename}' to Recycle Bin"); self.refresh_file_list(list_widget.category_name)
            except Exception as e:
                self.log(f"ERROR: Could not move to Recycle Bin. See console for details.")
                print("--- SEND2TRASH ERROR ---"); print(f"Failed to trash file: {path}"); traceback.print_exc(); print("------------------------")
